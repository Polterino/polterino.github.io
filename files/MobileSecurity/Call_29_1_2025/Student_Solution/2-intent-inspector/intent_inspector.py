...
def parse_logs():
    with open("./intent_inspector.txt") as f:
        print(f.read())


def print_logs():
    f = open("intent_inspector.txt", "w")
...
def uninstall(apk):
    if (
        os.system(
            "adb shell pm list packages | grep {package}".format(
...
def install(apk):
    ...
    while True:
        try:
            os.system("adb install -g -t {apk}".format(apk=apk.get_filename()))
            break
...
def parse_args():
    ...
    parser.add_argument(
        "victimapp_apk_path", help="path to the victim app apk file"
    )
    parser.add_argument(
        "malapp_apk_path", help="path to the malicious app apk file"
    )
...
def main(args):
    ...
    # check if emulators or devices are online
    devices = subp.check_output(["adb", "devices", "-l"], text=True)
    devices_online = 0
